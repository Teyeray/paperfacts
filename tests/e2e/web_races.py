...
async def check_page(page: Page, base: str, docs: dict[str, str], _: Path) -> None:
    ...
    expect(await page.locator("#check-result .check-fields tbody tr").count() == len(data["fields"]), "fields missing")
    expect(await page.locator("#check-result details.check-prompt").count() >= 3, "the prompts are not previewed")
