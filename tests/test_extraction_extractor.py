...
from paperfacts.extraction.extractor import ContextBudgetError, extract_lane, extractor_key
from paperfacts.extraction.fields import schema_fingerprint
from paperfacts.extraction.llm import LlmResponseError, LlmResult
from paperfacts.fingerprint import FINGERPRINT_LENGTH
...
def test_extractor_key_is_stable_for_the_same_inputs():
    ...
    assert extractor_key("deepseek-chat") == extractor_key("deepseek-chat")
...
def test_extractor_key_changes_with_the_model():
    ...
    assert extractor_key("deepseek-chat") != extractor_key("gpt-4o")
...
def test_extractor_key_is_unaffected_by_the_default_number_of_passes():
    ...
    assert extractor_key("deepseek-chat") == extractor_key("deepseek-chat", passes=1)
...
def test_extractor_key_changes_when_the_number_of_passes_is_not_one():
    ...
    assert extractor_key("deepseek-chat") != extractor_key("deepseek-chat", passes=3)
    assert extractor_key("deepseek-chat", passes=2) != extractor_key("deepseek-chat", passes=3)


def test_extractor_key_changes_when_the_cleaning_fingerprint_changes(monkeypatch):
    # records.py and passes.py decide which of the model's claims survive; changing either must
    ...
    monkeypatch.setattr("paperfacts.extraction.extractor.cleaning_fingerprint", lambda: "aaaaaaaaaaaa")
    before = extractor_key("deepseek-chat")
    monkeypatch.setattr("paperfacts.extraction.extractor.cleaning_fingerprint", lambda: "bbbbbbbbbbbb")
    after = extractor_key("deepseek-chat")
...
def test_a_valid_response_becomes_a_lane_extraction_in_one_call():
    ...
    lane = extract_lane(artifact, client)
...
def test_the_usage_and_the_raw_response_are_kept_as_evidence():
    ...
    lane = extract_lane(make_artifact(), client)
...
def test_the_lane_records_the_extractor_key_and_the_schema_fingerprint():
    ...
    assert lane.extractor_key == extractor_key("some-model")
...
def test_the_markdown_of_the_artifact_is_what_reaches_the_model():
    ...
    extract_lane(artifact, client)

    assert artifact.markdown in client.users[0]
...
def test_the_values_reach_the_lane_un_normalized():
    ...
    lane = extract_lane(make_artifact(), FakeLlmClient([response_json()]))
...
def test_both_lanes_get_a_byte_identical_system_prompt():
    ...
    extract_lane(mineru, client_a)
    extract_lane(paddle, client_b)
...
def test_the_repair_request_carries_the_validation_error_and_the_previous_answer():
    ...
    extract_lane(make_artifact(), client)
...
def test_the_repair_restates_the_original_request():
    ...
    assert artifact.markdown in client.users[1]
...
def test_refresh_is_forwarded_to_the_client_so_force_really_re_asks_the_model():
    ...
    extract_lane(make_artifact(), client, refresh=True)
...
def test_passes_greater_than_one_makes_that_many_model_calls():
    ...
    lane = extract_lane(make_artifact(), client, passes=3)
...
def test_each_pass_after_the_first_gets_its_own_cache_salt():
    ...
    extract_lane(make_artifact(), client, passes=3)
...
def test_passes_must_be_at_least_one():
    with pytest.raises(ValueError, match="at least 1"):
        extract_lane(make_artifact(), FakeLlmClient([]), passes=0)
...
def test_an_oversized_document_is_rejected_before_any_model_call_is_made():
    ...
    with pytest.raises(ContextBudgetError):
        extract_lane(make_artifact(), client, context_tokens=100)
...
def test_the_context_budget_error_names_the_ways_to_fix_it():
    with pytest.raises(ContextBudgetError, match="PAPERFACTS_LLM_CONTEXT_TOKENS"):
        extract_lane(make_artifact(), FakeLlmClient([]), context_tokens=100)
...
def test_the_known_ids_come_from_the_artifact_blocks():
    ...
    lane = extract_lane(paddle, client)
...
def test_a_response_that_found_nothing_is_a_valid_empty_lane():
    lane = extract_lane(make_artifact(), FakeLlmClient(['{"target": null, "samples": []}']))
...
def test_tuning_the_matching_prompt_does_not_invalidate_extractions(monkeypatch):
    ...
    """
    before = extractor_key("deepseek-chat")
    monkeypatch.setattr("paperfacts.extraction.prompts.matching_system_prompt", lambda: "something else")

    assert extractor_key("deepseek-chat") == before


def test_tuning_the_matching_prompt_does_invalidate_comparisons(monkeypatch):
    from paperfacts.consensus import comparison_key

    comparison_key.cache_clear() if hasattr(comparison_key, "cache_clear") else None
    before = comparison_key()
    monkeypatch.setattr("paperfacts.consensus.compare.matching_system_prompt", lambda: "something else")
