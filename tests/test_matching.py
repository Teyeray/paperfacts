...
def test_identical_sample_ids_pair_up_without_calling_the_model(tco_profile):
    lane_a, lane_b = lanes(["A", "B"], ["A", "B"])
    client = FakeLlmClient([])

    matching = match_samples(lane_a, lane_b, client, tco_profile)
...
def test_exact_pairs_are_full_confidence_and_say_why(tco_profile):
    lane_a, lane_b = lanes(["A"], ["A"])

    matching = match_samples(lane_a, lane_b, FakeLlmClient([]), tco_profile)

    assert matching.pairs[0].confidence == 1.0
    assert matching.pairs[0].justification == "identical sample_id"


def test_ids_that_differ_only_in_case_spacing_or_decoration_still_pair_exactly(tco_profile):
...
def test_a_hyphen_or_a_space_between_tokens_is_only_a_separator(tco_profile):
...
def test_ids_that_name_different_samples_never_pair_exactly(a, b, tco_profile):
...
def test_no_model_call_when_the_exact_pass_consumed_one_side_entirely(tco_profile):
...
def test_an_empty_lane_means_everything_is_unmatched_and_no_model_call(tco_profile):
    lane_a, lane_b = lanes(["A", "B"], [])
    client = FakeLlmClient([])
...
def test_two_empty_lanes_produce_an_empty_matching(tco_profile):
    lane_a, lane_b = lanes([], [])
...
def test_the_model_pairs_the_samples_that_the_exact_pass_could_not(tco_profile):
...
def test_the_model_usage_and_raw_answer_are_kept(tco_profile):
    lane_a, lane_b = lanes(["A1"], ["B1"])
...
def test_exact_and_model_pairs_are_counted_together(tco_profile):
...
def test_only_the_leftovers_are_shown_to_the_model(tco_profile):
    ...
    match_samples(lane_a, lane_b, client, tco_profile)
...
def test_a_pair_naming_an_id_that_does_not_exist_is_ignored(tco_profile):
...
def test_a_second_pair_reusing_an_id_is_ignored(tco_profile):
...
def test_an_unparseable_answer_gets_one_repair_attempt(tco_profile):
...
def test_two_unparseable_answers_mark_the_matching_as_failed_without_raising(tco_profile):
...
def test_an_offline_miss_is_not_a_failed_matching(tco_profile):
    ...
    with pytest.raises(LlmOfflineMiss):
        match_samples(lane_a, lane_b, FakeLlmClient(miss), tco_profile)


def test_a_failed_matching_still_keeps_the_exact_pairs(tco_profile):
...
def test_an_answer_with_an_out_of_range_confidence_is_treated_as_unparseable(tco_profile):
    lane_a, lane_b = lanes(["A1"], ["B1"])
...
def test_a_pair_without_a_confidence_gets_a_neutral_default(tco_profile):
    lane_a, lane_b = lanes(["A1"], ["B1"])
...
def test_the_unmatched_lists_from_the_model_are_not_trusted_verbatim(tco_profile):
...
def test_the_prompt_lists_the_id_label_conditions_and_raw_field_values(tco_profile):
...
def test_a_sample_without_label_conditions_or_fields_renders_placeholders(tco_profile):
...
def test_both_backend_names_reach_the_prompt(tco_profile):
...
def test_refresh_is_forwarded_to_the_client(tco_profile):
    lane_a, lane_b = lanes(["A1"], ["B1"])
    client = FakeLlmClient([matching_json()])

    match_samples(lane_a, lane_b, client, tco_profile, refresh=True)

    assert client.refreshes == [True]


def test_refresh_defaults_to_false(tco_profile):
    lane_a, lane_b = lanes(["A1"], ["B1"])
    client = FakeLlmClient([matching_json()])
