...
in the cache entry -- the entry stores the answer, and the image lives where it was rendered from.
"""
...
class VisionClient(Protocol):
    """What the validation stage needs from a vision-language model: one image plus a question in, text out.

    Deliberately a separate protocol from :class:`LlmClient`. Extraction must never be handed an image (the
    two lanes are compared on *text* the parsers produced), and validation must never be handed a text-only
    client and silently degrade to re-reading parser output. Same three attributes for the same reason: they
    decide what the model was asked and belong in ``validation_key``.
    """
...
class OpenAICompatibleClient:
    ...
    def complete_vision(self, *, system: str, user: str, image_png: bytes, refresh: bool = False) -> LlmResult:
        ...
        validation stage parses the reply leniently anyway, so the request stays portable across vLLM and
    ...
    def vision_payload(self, *, system: str, user: str, image_png: bytes) -> dict[str, Any]:
        ...
        vLLM / Model Studio imitations all accept it."""
    ...
    def vision_cache_key(self, payload: dict[str, Any], image_png: bytes) -> str:
        ...
        image and the on-disk cache would carry a copy of every crop ever sent. The digest is exact -- the
        same PNG bytes give the same key, one different pixel gives another -- so nothing is lost.
        """
            ...
            if isinstance(message.get("content"), list):
                for part in message["content"]:
                    if part.get("type") == "image_url":
                        part["image_url"] = {"sha256": hashlib.sha256(image_png).hexdigest()}
