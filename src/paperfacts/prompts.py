...
# lost 36 substrate temperatures this way). Shared so the two modes place a subset the same way.
_SUBSET_SCOPE = (
    'A value the paper states for a named subset of the listed samples -- "all films deposited at 100 °C",'
    ' "the annealed films", "the samples on glass" -- belongs to each sample in that subset: report it once'
    " per sample of the subset, each time under that sample's own id, never as one entry without a sample id."
    " `applies_to_all_samples` stays reserved for a value that holds for the whole list; a subset, however"
    " large, is reported sample by sample."
)
