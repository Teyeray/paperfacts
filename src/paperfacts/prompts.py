...
4. `sample_id` must be copied exactly from the sample list in the question. Use null only for a paper-level field, or when the excerpts genuinely do not say which sample the value belongs to. If the list holds exactly one sample, every sample-level value belongs to it. When the excerpt names the sample by a description instead of its id -- a label, a condition or a property such as "the electrode with 0.46 atom % V" or "the film annealed at 450 °C" -- match that description against the label and conditions of each list entry, and if it fits exactly one entry, copy that entry's id. If it fits several entries or none, leave `sample_id` null; never invent or adapt an id.
