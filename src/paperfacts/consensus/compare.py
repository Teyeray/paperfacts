...
per-field tolerances configured in :mod:`paperfacts.extraction.fields`; text/composition comparison uses
...
from paperfacts.extraction.fields import SAMPLE_FIELDS, TARGET_FIELDS, FieldSpec, schema_fingerprint
from paperfacts.extraction.prompts import matching_system_prompt
...
from paperfacts.normalization import normalization_fingerprint, normalize_lane
from paperfacts.normalization.text import normalize_key
from paperfacts.normalization.units import clean_unit

FactStatus = Literal["agree", "conflict", "ambiguous", "missing"]
# Sample-pairing confidence below this threshold counts as "low confidence" (per the PRD: such records
# should be downgraded to AMBIGUOUS for review)
AMBIGUOUS_MATCH_CONFIDENCE = 0.6
...
class ComparisonCounts(BaseModel):
    """The evaluation counts the PRD asks for; every key is present even at zero, so consumers never need
    to guard against KeyError."""
...
def comparison_key() -> str:
    """Fingerprint of everything a comparison depends on: tolerances, the low-confidence threshold, the
    normalisation rules, and the prompt that pairs samples across the two lanes.

    All of these are cheap to recompute -- none of them re-runs the per-lane extraction -- so tuning a
    tolerance or the matching prompt costs nothing but never serves a stale verdict either.
    """
...
def compare_lanes(lane_a: LaneExtraction, lane_b: LaneExtraction, matching: SampleMatching) -> ComparisonReport:
    ...
    return ComparisonReport(
        ...
        counts=_count(comparisons, matching),
    )
...
def _pair_values(
    ...
    """Pair up both sides' values for the same field: exact-match by normalized condition first; remaining
    numeric values are then greedily paired by numeric proximity; anything left over is one-sided."""
    by_key_a, by_key_b = _by_condition(values_a), _by_condition(values_b)
    pairs: list[tuple[FieldValue | None, FieldValue | None]] = [
        (by_key_a[key], by_key_b[key]) for key in sorted(by_key_a.keys() & by_key_b.keys())
    ]
    rest_a = [v for k, v in by_key_a.items() if k not in by_key_b]
    rest_b = [v for k, v in by_key_b.items() if k not in by_key_a]
    if spec.kind == "numeric":
...
def _by_condition(values: Sequence[FieldValue]) -> dict[str, FieldValue]:
    """Index values by normalized condition; when a condition appears more than once, keep the first."""
    keyed: dict[str, FieldValue] = {}
    for value in values:
        keyed.setdefault(normalize_key(value.condition), value)
    return keyed
...
def _count(comparisons: Sequence[FieldComparison], matching: SampleMatching) -> ComparisonCounts:
